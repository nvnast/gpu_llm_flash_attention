"""
Flash Attention implementation detail tests.

This module tests Flash Attention internals specific to the algorithm:
- Verifies the log-sum-exp (L) tensor is computed and saved correctly
- Tests that the implementation uses ctx.save_for_backward properly

For general correctness tests (forward/backward match reference), see test_flash_attention.py.
...
def _run_triton_flash_attention(impl, q, k, v, is_causal):
    """Wrapper to handle Triton compilation errors on unsupported GPU architectures (T4/Turing)."""
    return skip_on_triton_error(impl, q, k, v, is_causal)
...
def test_flash_forward_pass_triton(is_causal):
    impl = get_flashattention_autograd_function_triton().apply
    wrapped_impl = lambda q, k, v, causal: _run_triton_flash_attention(impl, q, k, v, causal)
...
def test_flash_backward_triton(is_causal):
    ...
    impl = get_flashattention_autograd_function_triton().apply
    output = _run_triton_flash_attention(impl, q, k, v, is_causal)
    output.backward(do)
